        ...
        for account in account_groups:
            ...
            account_rule_lines.extend(
                [
                    ...
                    (
                        "- 기준일 의미: 정기적으로 매수 여부를 검토하는 날짜이며, "
                        "매수 의무일이나 해당 월의 매수 마감일이 아님"
                    ),
                    (
                        f"- 시스템 계산 매수 가능 범위: "
                        f"{rec_desc}"
...
17. '정기 매수 검토 기준일'은 매수를 반드시 실행해야 하는 날짜가 아니라 정기적으로 매수 여부를 검토하는 기준점입니다.
18. 기준일이 지났다는 이유만으로 대기, 기존 계획 유지 또는 매수 불가로 판단하지 마세요. 같은 월의 남은 매수 가능 범위가 있다면 기준일 이후에도 시장 상황과 포트폴리오 상태를 다시 분석하여 매수, 일부 매수, 대기 또는 기존 계획 유지를 판단할 수 있습니다.
19. 기준일 이전이라고 해서 무조건 기다릴 필요도 없습니다. 시장 상황과 사용자 투자 원칙상 합리적인 근거가 있다면 남은 매수 가능 범위 안에서 매수 또는 일부 매수를 제시할 수 있습니다.
20. 경제지표와 시장지표의 기준 시점을 반드시 구분하세요. 주가, 국채금리, 환율, 유가와 같은 시장지표와 근원 PCE, 비농업 신규고용(NFP), 실업률 같은 월간 경제지표는 서로 다른 시점의 데이터일 수 있습니다.
21. 근원 PCE, 비농업 신규고용(NFP), 실업률 등 월간 경제지표에 표시된 '기준' 또는 '기준월'을 해당 경제지표의 관측 시점으로 해석하세요.
22. 경제지표의 기준월이 현재월보다 이전이라는 이유만으로 해당 데이터를 오래되었거나 신뢰도가 낮은 데이터라고 판단하지 마세요. 공식기관이 현재 제공하는 최신 발표치일 수 있습니다.
23. 서로 다른 기준월의 경제지표를 비교하거나 현재 금융시장과 연결해 해석할 때는 발표 및 관측 시점의 차이를 고려하세요. 예를 들어 7월 물가지표와 8월 고용지표를 동일한 시점의 경기 상태를 측정한 것처럼 단순 비교하지 마세요.
24. 최신 시장가격이 이전 기준월의 경제지표 발표 이후 이미 변화했을 수 있다는 점을 고려하세요. 과거 기준월 경제지표 하나만으로 현재 시장 움직임의 원인을 단정하지 마세요.
25. 경제지표의 방향성을 해석할 때 수치뿐 아니라 해당 지표의 기준월을 함께 확인하세요. 필요한 경우 분석 문장에서 '7월 근원 PCE', '8월 고용지표'처럼 기준 시점을 명시하여 설명하세요.
26. 미국 10년물 국채금리의 변화량은 제공된 bp(basis point) 단위를 그대로 해석하세요. 예를 들어 '+20bp'는 금리가 20% 상승했다는 뜻이 아니라 금리 수준이 0.20%포인트 상승했다는 의미입니다.
27. '데이터 없음' 또는 '조회 실패'로 표시된 지표는 투자 판단의 근거로 사용하지 마세요. 누락된 값을 추정하거나 다른 값으로 임의 보완하지 마세요.
